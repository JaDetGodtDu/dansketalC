...
void convert_thousands(int num, char *buffer, DanskeTalConfig *config)
{
    if (num >= 1000 && num <= 999999)
    {
        ...
        if (remainder != 0)
        {
            ...
            if (remainder < 10)
            {
                convert_units(remainder, buffer, 1, config);
            }
            else if (remainder < 100)
            {
                convert_tens(remainder, buffer, config);
            }
            else
            {
                convert_hundreds(remainder, buffer, config);
            }
        }
    }
}
...
void convert_millions(int num, char *buffer, DanskeTalConfig *config)
{
    if (num >= 1000000)
    {
        ...
        if (remainder != 0)
        {
            ...
            {
                convert_thousands(remainder, buffer, config);
            }
        }
    }
}